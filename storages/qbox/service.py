...
def check_valid_bucket_name(bucket_name):
    """
        验证bucketname是否符合要求，仅限[a-zA-Z0-9_]组合
    """
    return re.compile('\w+').match(bucket_name)
...
class QboxService:
    ...
    def __init__(self, conn, bucket=config.BUCKET_NAME):
        self.Conn = conn
    ...
    def PutAuth(self):
        ...
        return self.Conn.make_request(url)
    ...
    def SaveAs(self, key, source_url, opWithParams):
        ...
        url = source_url + "?" + opWithParams + saveAsParam
        return self.Conn.make_request(url)
